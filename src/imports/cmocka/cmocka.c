...
static void add_symbol_value(
    ListNode *const symbol_map_head,
...
static void remove_always_return_values(
    ListNode *const map_head, const size_t number_of_symbol_names);

static size_t check_for_leftover_values_list(
    const ListNode *head, const char *const error_message);

static size_t check_for_leftover_values(
    const ListNode *const map_head,
...
static void set_source_location(
    SourceLocation *const location, const char *const file, const int line)
{
  assert_non_null(location);
  location->file = file;
  location->line = line;
...
static void teardown_testing(const char *test_name)
{
  (void) test_name;
  list_free(
      &global_function_result_map_head, free_symbol_map_value, (void *) 0);
  initialize_source_location(&global_last_mock_value_location);
  list_free(
      &global_function_parameter_map_head, free_symbol_map_value, (void *) 1);
  initialize_source_location(&global_last_parameter_location);
...
{
  return !strcmp(
      ((SymbolMapValue *) map_value)->symbol_name, (const char *) symbol);
}
    ...
    const int refcount)
{
  ...
  if (!list_find(
          symbol_map_head, symbol_name, symbol_names_match, &target_node)) {
  ...
  if (number_of_symbol_names == 1) {
    ...
    add_symbol_value(
        &target_map_value->symbol_values_list_head,
        &symbol_names[1],
    ...
    if (number_of_symbol_names == 1) {
      ...
      *output = (void *) value_node->value;
      ...
      return_value = get_symbol_value(
          child_list, &symbol_names[1], number_of_symbol_names - 1, output);
    }
    ...
    const ListNode *head, const char *const error_message)
{
  ...
  if (!list_empty(head)) {
    for (child_node = head->next; child_node != head;
      ...
      cm_print_error(
          SOURCE_LOCATION_FORMAT ": note: remaining item was declared here\n",
          o->location.file,
          o->location.line);
    }
  }
    ...
    if (!list_empty(child_list)) {
      if (number_of_symbol_names == 1) {
        ...
        check_for_leftover_values(
            child_list, error_message, number_of_symbol_names - 1);
      }
    ...
    const int count)
{
  ...
  add_symbol_value(
      &global_function_result_map_head, &function_name, 1, return_value, count);
}
...
static int
float_compare(const float left, const float right, const float epsilon)
{
  float absLeft;
  float absRight;
  float largest;
...
/* Returns 1 if the specified float values are equal. If the values are not
 * equal an error is displayed and 0 is returned. */
static int float_values_equal_display_error(
    const float left, const float right, const float epsilon)
{
  const int equal = float_compare(left, right, epsilon);
  if (!equal) {
    cm_print_error(
        FloatPrintfFormat " != " FloatPrintfFormat "\n", left, right);
  }
  return equal;
}

/* Returns 1 if the specified float values are different. If the values are
 * equal an error is displayed and 0 is returned. */
static int float_values_not_equal_display_error(
    const float left, const float right, const float epsilon)
{
...
static int values_equal_display_error(
    const LargestIntegralType left, const LargestIntegralType right)
{
 ...
 * an error is displayed and 0 is returned. */
static int values_not_equal_display_error(
    const LargestIntegralType left, const LargestIntegralType right)
{
    ...
    const int invert)
{
  ...
  assert_non_null(check_integer_set);
  {
    ...
    cm_print_error(
        LargestIntegralTypePrintfFormatDecimal " is %sin the set (",
        value,
        invert ? "" : "not ");
    for (i = 0; i < size_of_set; i++) {
 ...
 */
static int memory_equal_display_error(
    ...
    const LargestIntegralType value, const LargestIntegralType check_value_data)
{
  CheckIntegerRange *const check_integer_range =
      cast_largest_integral_type_to_pointer(
          CheckIntegerRange *, check_value_data);
...
{
  expect_range(
      function, parameter, file, line, minimum, maximum, check_in_range, count);
}
...
{
  _expect_check(
      function, parameter, file, line, check_value, value, NULL, count);
}

/* CheckParameterValue callback to check whether a value is not equal to an
 * expected value. */
static int check_not_value(
    const LargestIntegralType value, const LargestIntegralType check_value_data)
{
...
{
  _expect_check(
      function, parameter, file, line, check_not_value, value, NULL, count);
}
    ...
    const int count)
{
  declare_initialize_value_pointer_pointer(
      string_pointer, discard_const(string));
  _expect_check(
      function,
      parameter,
      file,
...
{
  expect_memory_setup(
      function, parameter, file, line, memory, size, check_memory, count);
}

/* CheckParameterValue callback to check whether a parameter is not equal to
 * an area of memory. */
static int check_not_memory(
    const LargestIntegralType value, const LargestIntegralType check_value_data)
{
  CheckMemoryData *const check = cast_largest_integral_type_to_pointer(
      CheckMemoryData *, check_value_data);
...
{
  expect_memory_setup(
      function, parameter, file, line, memory, size, check_not_memory, count);
}
    ...
    const LargestIntegralType value)
{
  ...
  const int rc = get_symbol_value(
      &global_function_parameter_map_head, symbols, 2, &result);
  if (rc) {
    ...
    const int line)
{
  ...
  if (result > valmax - 1) {
    if (error > 0) {
      cm_print_error(
          "%s < 0, errno(" LargestIntegralTypePrintfFormatDecimal "): %s\n",
          expression,
          error,
          strerror((int) error));
    } else {
...
void _assert_string_not_equal(
    const char *const a, const char *const b, const char *file, const int line)
{
...
void _assert_memory_not_equal(
    ...
    const int line)
{
  if (!memory_not_equal_display_error(
          (const char *) a, (const char *) b, size)) {
    _fail(file, line);
  }
}
...
void *_test_malloc(const size_t size, const char *file, const int line)
{
  ...
  ptr =
      (char
           *) (((size_t) block + MALLOC_GUARD_SIZE + sizeof(struct MallocBlockInfoData) + MALLOC_ALIGNMENT) & ~(MALLOC_ALIGNMENT - 1));
  ...
  block_info.ptr =
      ptr - (MALLOC_GUARD_SIZE + sizeof(struct MallocBlockInfoData));
...
void _test_free(void *const ptr, const char *file, const int line)
{
  ...
  block_info.ptr =
      block - (MALLOC_GUARD_SIZE + sizeof(struct MallocBlockInfoData));
  /* Check the guard blocks. */
  {
    char *guards[2] = {
        block - MALLOC_GUARD_SIZE, block + block_info.data->size};
...
static void free_allocated_blocks(const ListNode *const check_point)
{
  ...
  while (node != head) {
    ...
    node = node->next;
    free(
        discard_const_p(char, block_info.data) +
...
void _fail(const char *const file, const int line)
{
  ...
  switch (output) {
    case CM_OUTPUT_STDOUT:
      cm_print_error(
          "[   LINE   ] --- " SOURCE_LOCATION_FORMAT ": error: Failure!",
          file,
          line);
      break;
    default:
...
static void cmprintf_group_finish_xml(
...
static void cmprintf_group_finish_standard(
    size_t total_executed,
    size_t total_passed,
    size_t total_failed,
    size_t total_errors,
...
static void cmprintf_standard(
    enum cm_printf_type type, const char *test_name, const char *error_message)
{
  switch (type) {
...
static void cmprintf_group_finish_tap(
    const char *group_name,
    size_t total_executed,
    size_t total_passed,
    size_t total_skipped)
...
static void cmprintf_tap(
    enum cm_printf_type type,
...
static void cmprintf_group_finish(
    const char *group_name,
    size_t total_executed,
    size_t total_passed,
    size_t total_failed,
  ...
  switch (output) {
    ...
    case CM_OUTPUT_TAP:
      cmprintf_group_finish_tap(
          group_name, total_executed, total_passed, total_skipped);
      break;
    case CM_OUTPUT_XML:
 ...
 * CMOCKA TEST RUNNER
 ****************************************************************************/
static int cmocka_run_one_test_or_fixture(
    ...
    const void *const heap_check_point)
{
  const ListNode *const volatile check_point =
      (const ListNode
           *) (heap_check_point != NULL ? heap_check_point : check_point_allocated_blocks());
...
static int cmocka_run_group_fixture(
    const char *function_name,
    CMFixtureFunction setup_func,
    CMFixtureFunction teardown_func,
    void **state,
    const void *const heap_check_point)
{
  ...
  if (setup_func != NULL) {
    rc = cmocka_run_one_test_or_fixture(
        function_name, NULL, setup_func, NULL, state, heap_check_point);
  } else {
    rc = cmocka_run_one_test_or_fixture(
    ...
    CMFixtureFunction group_teardown)
{
  ...
  for (i = 0; i < num_tests; i++) {
    if (tests[i].name != NULL &&
        (tests[i].test_func != NULL || tests[i].setup_func != NULL ||
  ...
  if (rc == 0) {
    ...
    for (i = 0; i < total_tests; i++) {
      ...
      if (rc == 0) {
        ...
        cmprintf(PRINTF_TEST_ERROR, test_number, cmtest->test->name, err_msg);
        total_errors++;
      }
    }
  } else {
    ...
    const void *const heap_check_point)
{
  const ListNode *const volatile check_point =
      (const ListNode
           *) (heap_check_point ? heap_check_point : check_point_allocated_blocks());
  void *current_state = NULL;
  volatile int rc = 1;
  int handle_exceptions = 1;
#ifdef _WIN32
...
int _run_tests(const UnitTest *const tests, const size_t number_of_tests)
{
  ...
  if (number_of_test_states != 0) {
    print_error(
        "[  ERROR   ] Mismatched number of setup %" PRIdS
        " and "
...
int _run_group_tests(const UnitTest *const tests, const size_t number_of_tests)
{
  ...
  while (current_test < number_of_tests) {
    ...
    if (run_test) {
      ...
      failed = _run_test(
          test->name, test->function, current_state, test->function_type, NULL);
      if (failed) {
        failed_names[total_failed] = test->name;
      }
