...
void sec_microdog40d_run(void)
{
  ...
  retval = bind(fd, (struct sockaddr *) &peer_addr, sizeof(struct sockaddr_un));
  ...
  while (sec_microdog40d_run_d) {
    ...
    retval = recvfrom(
        fd,
        &transaction,
        sizeof(struct sec_microdog40_transaction),
        0,
    ...
    if (retval == -1) {
      ...
      log_warn(
          "Package too short, expected %d bytes, got %d bytes",
          sizeof(struct sec_microdog40_transaction),
          retval);
    } else {
