...
sec_hasp_handler_alloc_transaction(size_t max_payload_size)
{
  ...
  trans->req_resp_max_size =
      sizeof(struct sec_hasp_handler_header) + max_payload_size;
...
{
  log_debug(
      "Req: packet_size %d, head_value 0x%X, transaction_num %d, session_id "
      "0x%X, packet_type 0x%X, "
      "unknown 0x%X, tail_value 0x%X",
      transaction->req->header.packet_size,
      transaction->req->header.head_value,
      transaction->req->header.transaction_num,
      transaction->req->header.session_id,
      transaction->req->header.packet_type,
      transaction->req->header.tail_value);
  ...
  switch (transaction->req->header.packet_type) {
    ...
    default:
      log_error(
          "Invalid packet type: %X", transaction->req->header.packet_type);
      memset(
          &transaction->req->header, 0, sizeof(struct sec_hasp_handler_header));
      break;
  }
}

// static void sec_hasp_handler_get_client_id(struct
// sec_hasp_handler_transaction* transaction)
//{
