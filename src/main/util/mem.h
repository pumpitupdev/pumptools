...
/**
 * Install a different memory interfacee, e.g. to use a different allocator or
 * hooks for unit-testing.
 */
void util_mem_init(const struct util_mem_interface *interface);

/**
 * Optional, initialize the default memory interface. This needs to be called
 * only if you want to switch back from a non-default interface that got
 * installed using util_mem_init.
 */
void util_mem_init_default();

/**
 * Enable debug output of all allocations and de-allocations that go through the
 * default interface of this module.
 */
 ...
 *
 * @param mem Pointer to the pointer with allocated memory to free. The pointer
 * to the memory will be set to NULL.
 */
