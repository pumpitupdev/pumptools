...
static bool util_options_init_from_args(
    struct util_options_opts *options,
    const struct util_options_defs *option_defs,
    int argc,
    char **argv);
static bool util_options_init_from_file(
    struct util_options_opts *options,
    const struct util_options_defs *option_defs,
    char *file_path);
...
util_options_get(const struct util_options_defs *option_defs)
{
  ...
  options =
      util_options_get2(util_options_argc, util_options_argv, option_defs);
...
int util_options_get_int(const struct util_options_opts *opts, const char *name)
{
    ...
    int argc, char **argv, const struct util_options_defs *option_defs)
{
  ...
  struct util_options_opts *options = (struct util_options_opts *) util_xmalloc(
      sizeof(struct util_options_opts));
  ...
  memset(
      options->values, 0, sizeof(struct util_options_value) * options->entries);
  ...
  if (options_file) {
    ...
    success = util_options_init_from_file(options, option_defs, options_file);
          ...
          case UTIL_OPTIONS_TYPE_BIN: {
            ...
            if (strlen(argv[i]) > 0) {
              ...
              if (!util_hex_decode(
                      ...
                      options->values[j].value.bin.len,
                      argv[i],
                      strlen(argv[i]))) {
                log_warn(
                    "Decoding binary data for key %s "
                    "failed",
    ...
    char *file_path)
{
  ...
  if (!util_file_load(file_path, (void **) &data, &len, true)) {
    ...
    if (util_fs_path_exists(file_path)) {
      log_warn(
          "Loading options file %s failed, faulty or corrupted "
          "configuration",
          file_path);
    } else {
      ...
      if (option_defs->usage_param) {
        log_info(
            "Options file %s does not exist, creating default...", file_path);

        if (!util_options_write_default_options_file(option_defs, file_path)) {
          log_warn("Creating default options file %s failed", file_path);
        } else {
          log_info(
              "Options file %s created. Tweak it to your needs and restart the "
              "game.",
              file_path);
        }
      }
  ...
  while (pos_lines != NULL) {
    ...
    if (strlen(pos_lines) > 0 && pos_lines[0] != '#') {
      ...
      if (pos_key_val != NULL) {
        ...
        log_warn(
            "Invalid options line %s in options file %s", pos_lines, file_path);
        free(pos_line_dup);
        free(data);
        return false;
      }
      ...
      for (uint32_t j = 0; j < options->entries; j++) {
        ...
        if (!strcmp(options->defs->defs[j].name, key)) {
          ...
          switch (options->defs->defs[j].type) {
            ...
            case UTIL_OPTIONS_TYPE_STR: {
              if (val && strlen(val) > 0) {
                options->values[j].value.str = util_str_dup(val);
              } else {
                options->values[j].value.str = NULL;
              }
            ...
            case UTIL_OPTIONS_TYPE_BIN: {
              if (val && strlen(val) > 0) {
                ...
                options->values[j].value.bin.data = util_xmalloc(lenBin);
          ...
          log_debug(
              "Options file param: -%c %s (type %d): %s",
              options->defs->defs[j].param,
              options->defs->defs[j].name,
              options->defs->defs[j].type,
              val ?
    ...
    const struct util_options_defs *option_defs, char *file_path)
{
  ...
  for (uint32_t i = 0; i < option_defs->ndefs; i++) {
    ...
    switch (option_defs->defs[i].type) {
      ...
      default: {
        log_warn("Invalid value type %d found", option_defs->defs[i].type);
        break;
      }
    }
  }
