...
/**
 * Get the full path of the executable of the current process (including the
 * executable name).
 *
...
/**
 * Get the full path of the folder the executable of the current process is
 * located in.
 *
 * @param buffer Buffer to read the path into.
 * @param size Size of the buffer
...
/**
 * Special version of util_proc_get_folder_path_executable that considers when
 * the process is run explicitly with a ld-linux binary. Returns the path of the
 * application's executable and not ld-linux in that case.
 *
 * @param buffer Buffer to read the path into.
 * @param size Size of the buffer
 * @return True on success, false on failure, e.g. buffer too small.
