...
static void util_log_format(
    ...
    const char *fmt,
    va_list ap);
static void util_log_writer_console(
    void *ctx,
    enum util_log_level level,
    const char *module,
    const char *trace_file,
    size_t trace_line,
    const char *msg);
static void util_log_writer_file(
    void *ctx,
    enum util_log_level level,
    const char *module,
    const char *trace_file,
...
static void util_log_colorize_text(
    enum util_log_level level, const char *text, char *buffer)
{
  switch (level) {
    ...
    va_list ap)
{
  ...
  if (level <= util_log_level) {
    ...
    if (util_log_file != NULL) {
      util_log_writer_file(
          util_log_file, level, module, trace_file, trace_line, msg);
    }
  }
    ...
    const char *msg)
{
  ...
  util_str_format(
      text_level, sizeof(text_level), "[%c]", util_log_level_chars[level]);
    ...
    const char *msg)
{
  ...
  line_len = util_str_format(
      line,
      sizeof(line),
      "[%c][%d/%d/%d-%d:%d:%d:%d][%s][%s:%d] %s\n",
      util_log_level_chars[level],
      timestamp.year,
      timestamp.month,
      timestamp.day,
      timestamp.hour,
