...
static void nxahook_bootstrapping(
    int argc,
    char **argv,
...
static void nxahook_fs_redir_script(
    struct nxahook_options *options, const char *game_data_path)
{
  log_assert(options);
  log_assert(game_data_path);
...
static void nxahook_fs_redir_fonts(
    struct nxahook_options *options, const char *game_data_path)
{
  log_assert(options);
  log_assert(game_data_path);
...
static void nxahook_fs_redir_mission(
    struct nxahook_options *options, const char *game_data_path)
{
  log_assert(options);
  log_assert(game_data_path);
...
static void nxahook_fs_redirs_init(
    struct nxahook_options *options, const char *game_data_path)
{
  log_assert(options);
  log_assert(game_data_path);
...
static void nxahook_patch_dongle_init()
{
  patch_microdog40_init(
      ...
      ((uintptr_t) &_binary_nxahook_dog_key_end -
       (uintptr_t) &_binary_nxahook_dog_key_start));
}

static void nxahook_patch_hdd_check_init()
{
  patch_hdd_check_init(
      (const uint8_t *) _binary_nxahook_hdd_raw_start,
      ((uintptr_t) &_binary_nxahook_hdd_raw_end -
       (uintptr_t) &_binary_nxahook_hdd_raw_start));
}
