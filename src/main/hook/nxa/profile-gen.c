...
// Do not use util_fs_path_exists here as this fopens and causes an endless
// recursion
...
nxahook_profile_gen_filehook(struct cnh_filehook_irp *irp)
{
  // nx2hook_profile_gen_is_creating_file: Avoid recursion of any file calls
  // when creating the save/rank files
  if (irp->op != CNH_FILEHOOK_IRP_OP_OPEN ||
      nxahook_profile_gen_is_creating_file) {
  ...
  if (util_str_ends_with(irp->open_filename, "nxasave.bin") &&
    ...
    if (!asset_nxa_usb_save_save_to_file(irp->open_filename, save, true)) {
      log_error(
          "Saving generated nxasave.bin file to %s failed", irp->open_filename);
    }
  ...
  if (util_str_ends_with(irp->open_filename, "nxarank.bin") &&
    ...
    if (!asset_nxa_usb_rank_save_to_file(irp->open_filename, rank, true)) {
      log_error(
          "Saving generated nxarank.bin file to %s failed", irp->open_filename);
    }
