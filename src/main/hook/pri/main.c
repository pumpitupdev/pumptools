...
static void prihook_bootstrapping(int argc, char** argv, struct prihook_options* options, char** game_data_path)
{
    log_assert(argc >= 0);
    log_assert(argv);
    log_assert(options);
...
static void prihook_fs_redirs_init(struct prihook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
{
    patch_hasp_init((const uint8_t*) _binary_prihook_hasp_key_start,
        ((uintptr_t) &_binary_prihook_hasp_key_end - (uintptr_t) &_binary_prihook_hasp_key_start));
}

static void prihook_patch_hdd_check_init()
{
    patch_hdd_check_init((const uint8_t*) _binary_prihook_hdd_raw_start,
        ((uintptr_t) &_binary_prihook_hdd_raw_end - (uintptr_t) &_binary_prihook_hdd_raw_start));
}
...
static void prihook_patch_net_init(struct prihook_options* options)
{
    ...
    if (options->patch.net.server) {
        ...
        if (!util_net_split_ipv4_str(options->patch.net.server, &address, &port)) {
            ...
            patch_network_redirect_server_address("115.68.108.183", address, (uint16_t) port);
            free(address);
        }
    } else {
