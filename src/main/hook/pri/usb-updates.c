...
static enum cnh_result prihook_usb_updates_filehook(struct cnh_filehook_irp *irp)
{
    if (irp->op != CNH_FILEHOOK_IRP_OP_OPEN) {
        return cnh_filehook_invoke_next(irp);
    }

    if (!strcmp(irp->open_filename, "/mnt/0/prime_") || !strcmp(irp->open_filename, "/mnt/1/prime_")) {
