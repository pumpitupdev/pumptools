...
static void nxhook_bootstrapping(int argc, char** argv, struct nxhook_options* options, char** game_data_path)
{
    log_assert(argc >= 0);
    log_assert(argv);
    log_assert(options);
...
static void nxhook_fs_redir_script(struct nxhook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
static void nxhook_fs_redir_fonts(struct nxhook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
static void nxhook_fs_redirs_init(struct nxhook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
{
    patch_microdog34_init((const uint8_t*) _binary_nxhook_dog_key_start,
        ((uintptr_t) &_binary_nxhook_dog_key_end - (uintptr_t) &_binary_nxhook_dog_key_start));
}
