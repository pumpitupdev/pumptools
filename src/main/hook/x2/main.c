...
static void x2hook_bootstrapping(
    int argc,
    char **argv,
...
static void x2hook_fs_redir_script(
    struct x2hook_options *options, const char *game_data_path)
{
  log_assert(options);
  log_assert(game_data_path);
...
static void x2hook_fs_redirs_init(
    struct x2hook_options *options, const char *game_data_path)
{
  log_assert(options);
  log_assert(game_data_path);
...
static void x2hook_patch_dongle_init()
{
  patch_microdog34_init(
      (const uint8_t *) _binary_x2hook_dog_key_start,
      ((uintptr_t) &_binary_x2hook_dog_key_end -
       (uintptr_t) &_binary_x2hook_dog_key_start));
}
