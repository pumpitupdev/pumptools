...
#define PROHOOK_OPTIONS_STR_PATCH_PIUIO_EXIT_TEST_SERV "patch.piuio_exit.test_serv"
#define PROHOOK_OPTIONS_STR_PATCH_USB_PROFILE_P1_BUS_PORT "patch.usb_profile.p1.bus_port"
#define PROHOOK_OPTIONS_STR_PATCH_USB_PROFILE_P2_BUS_PORT "patch.usb_profile.p2.bus_port"
#define PROHOOK_OPTIONS_STR_PATCH_USB_PROFILE_DEV_NODES "patch.usb_profile.dev_nodes"
#define PROHOOK_OPTIONS_STR_PATCH_X11_EVENT_LOOP_INPUT_HANDLER "patch_x11_event_loop.input_handler_lib"
#define PROHOOK_OPTIONS_STR_PATCH_X11_EVENT_LOOP_INPUT_HANDLER2 "patch_x11_event_loop.input_handler_lib2"
...
static const struct util_options_def prohook_options_def[] = {
    ...
    },
    {
        ...
        .description = "Path to library implementing the piubtn api for piubtn emulation",
        .param = 'b',
        .type = UTIL_OPTIONS_TYPE_STR,
    ...
    },
    {
        ...
        .description = "Bus and port (format: X-X, e.g. 1-2) of the USB slot to assign to Player 1 when a USB thumb "
                       "drive is plugged in",
        .param = 'a',
        .type = UTIL_OPTIONS_TYPE_STR,
        .default_value.str = NULL,
    },
    {
        ...
        .description = "Bus and port (format: X-X, e.g. 1-2) of the USB slot to assign to Player 2 when a USB thumb "
                       "drive is plugged in",
        .param = 'k',
        .type = UTIL_OPTIONS_TYPE_STR,
        .default_value.str = NULL,
    },
    {
        ...
        .description = "Device nodes of plugged in USB thumb drives to consider for mounting for usb profiles, format: "
                       "sdX,sdY e.g. sde,sdf",
        .param = 't',
        .type = UTIL_OPTIONS_TYPE_STR,
        .default_value.str = NULL,
    },
    {
        ...
        .description = "Path to a second library implementing the x11-input-handler api to capture X11 keyboard inputs",
        .param = 'r',
        .type = UTIL_OPTIONS_TYPE_STR,
        .default_value.str = NULL,
    },
...
bool prohook_options_init(int argc, char** argv, struct prohook_options* options)
{
    ...
    options->game.data = util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_GAME_DATA);
    options->game.settings = util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_GAME_SETTINGS);
    options->patch.hook_mon.file = util_options_get_bool(options_opt, PROHOOK_OPTIONS_STR_PATCH_HOOK_MON_FILE);
    options->patch.hook_mon.fs = util_options_get_bool(options_opt, PROHOOK_OPTIONS_STR_PATCH_HOOK_MON_FS);
    options->patch.hook_mon.io = util_options_get_bool(options_opt, PROHOOK_OPTIONS_STR_PATCH_HOOK_MON_IO);
    options->patch.hook_mon.open = util_options_get_bool(options_opt, PROHOOK_OPTIONS_STR_PATCH_HOOK_MON_OPEN);
    options->patch.hook_mon.usb = util_options_get_bool(options_opt, PROHOOK_OPTIONS_STR_PATCH_HOOK_MON_USB);
    options->patch.piubtn.api_lib = util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_PIUBTN_EMU_LIB);
    options->patch.piuio.api_lib = util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_PIUIO_EMU_LIB);
    options->patch.piuio.exit_test_serv =
        util_options_get_bool(options_opt, PROHOOK_OPTIONS_STR_PATCH_PIUIO_EXIT_TEST_SERV);
    options->patch.usb_profile.device_nodes =
        util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_USB_PROFILE_DEV_NODES);
    options->patch.usb_profile.p1_bus_port =
        util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_USB_PROFILE_P1_BUS_PORT);
    options->patch.usb_profile.p2_bus_port =
        util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_USB_PROFILE_P2_BUS_PORT);
    options->patch.x11_event_loop.api_lib =
        util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_X11_EVENT_LOOP_INPUT_HANDLER);
    options->patch.x11_event_loop.api_lib2 =
        util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_X11_EVENT_LOOP_INPUT_HANDLER2);
    options->log.file = util_options_get_str(options_opt, PROHOOK_OPTIONS_STR_PATCH_UTIL_LOG_FILE);
    options->log.level =
        (enum util_log_level) util_options_get_int(options_opt, PROHOOK_OPTIONS_STR_PATCH_UTIL_LOG_LEVEL);
