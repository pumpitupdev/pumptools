...
ssize_t propatch_usb_fix_readlink(const char* path, char* buf, size_t len)
{
    ...
    // pro is scanning the /sys/block directory where all usb thumb drives show up once connected
    // The game iterates the directory and resolves the /sys/block/sdX/device symlink
    // On current kernels, e.g. 5.6.14, it might look like this
    ...
    // In order to allow the game to detect and mount usb thumb drives on newer kernels, the path
    // needs to be altered to match what the game is expecting which might be similar to this:
    // ../devices/pci0000:00/0000:00:14.0/usb2/2-2/2-2:1.0
    // The game tokenizes this path by / and picks the second to last item, which is '2-2' here.
    // This item gets further split into bus (2) and port (2). If that is successful, the game treats the
    // detected device as valid allowing it to mount if the bus and port match what has been configured in
    // the Static.ini file
    if (propatch_usb_fix_enabled && util_str_starts_with(path, "/sys/block/") && util_str_ends_with(path, "/device")) {
        log_debug("Trap %s", path);

        // Remove the trailing "/device" part which is no longer valid on current kernels
        // /sys/block/sdc/device -> /sys/block/sdc
        ...
        if (strstr(buf, propatch_usb_fix_device_thumb_drive_p1) || strstr(buf, propatch_usb_fix_device_thumb_drive_p2)) {
            ...
            if (    count != 12 ||
                    strcmp(toks[0], "..") != 0 ||
                    strcmp(toks[1], "devices") != 0 ||
                    strcmp(toks[10], "block") != 0) {
                log_error(
                    "/sys/block device readlink format changed, usb thumb drive profiles will likely not work!: %s",
                    buf);
            ...
            log_debug("Detected flash drive on bus %d, port %d, device %s", bus, port, device);

            for (int i = 0; i < 2; i++) {
                if (propatch_usb_fix_usb_bus[i] == bus && propatch_usb_fix_usb_port[i] == port) {
                    // The usb manager only cares about reading the second to last token in the correct format
...
void propatch_usb_fix_init(const char* device_nodes, const char* p1_bus_port, const char* p2_bus_port)
{
    ...
    if (sscanf(p1_bus_port, "%d-%d", &propatch_usb_fix_usb_bus[0], &propatch_usb_fix_usb_port[0]) != 2) {
        log_error("Invalid format for p1 bus-port: %s", p1_bus_port);
        return;
    }

    if (sscanf(p2_bus_port, "%d-%d", &propatch_usb_fix_usb_bus[1], &propatch_usb_fix_usb_port[1]) != 2) {
        log_error("Invalid format for p2 bus-port: %s", p2_bus_port);
        return;
    }

    log_info("Initialized: device nodes %s,%s, p1 %d-%d, p2 %d-%d",
