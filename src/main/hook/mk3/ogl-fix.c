 ...
 * multi-threaded mode for some reason and crash in
 * __glDispatchCheckMultithreaded when the game calls glXChooseVisual in its GFX
 * init function.
 *
 * The fix is quite simple (luckily) by just setting the environment variable
 * __GL_SINGLETHREADED.
 */
...
char *getenv(const char *name)
{
  ...
  if (!strcmp(name, "__GL_SINGLETHREADED")) {
    if (ret == NULL || strcmp(name, "1") != 0) {
      printf(
          "!!! mk3hook: __GL_SINGLETHREADED not set or enabled, force patching "
          "OpenGL to single "
