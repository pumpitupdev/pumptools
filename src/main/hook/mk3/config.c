...
int config_lookup_string(const config_t *config, const char *path, const char **value)
{
    ...
    if (!strcmp(path, "save_file")) {
        // Note: This is fine as the size of the string pointed to by the returned address is not relevant for any
        // operations using them, e.g. there is no merging/copying with limited stack'd buffer sizes involved like
        // below.
        *value = mk3hook_config_save_file;
    } else if (!strcmp(path, "effect_path")) {
        // Needs to be hardcoded because the stack buffer for this is only 128 bytes large. This is likely fine for most
        // paths and even when resolving relative paths to absolute ones. Still, this is a risk for a buffer overflow.
        // Utilize the redirect module in the main lib hook function to redirect this to something like a local game dir
        // and resolve this to an absolute path
        *value = "EFFECTS/";
    } else if (!strcmp(path, "track_path")) {
        // Needs to be hardcoded because the stack buffer for this is only 32 bytes large. When combining it with the
        // "Track%02i.mp3" part, the buffer will overflow when this path is longer or even resolved to an absolute path.
        // Utilize the redirect module in the main lib hook function to redirect this to something like a local game dir
        // and resolve this to an absolute path
        *value = "track/";
    } else {
...
static void mk3hook_config_assign_save_file(const struct mk3hook_options* options)
{
    ...
    // Don't free this one after assigning the result value as this will still be read from. Keep memory leak.
