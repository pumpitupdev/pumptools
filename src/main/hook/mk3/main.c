...
static void mk3hook_bootstrapping(int argc, char** argv, struct mk3hook_options* options, char** game_data_path)
{
    log_assert(argc >= 0);
    log_assert(argv);
    log_assert(options);
...
static void mk3hook_1st_2nd_fs_redirs(const char* game_data_path_abs)
{
    ...
    // The "datas" directory does not require a redirect as the game uses chdir to move to that folder. For this, the
    // additional patch module "1st-2nd-fs-fix" is required (see below). All files usually local to the piu executable,
    ...
    // For redirecting additional files for linux ports. Configurable via MK3 linux ports config.cfg but causes stack
    // overflow on long paths. Redirecting the paths afterwards is save.
...
static void mk3hook_3rd_and_up_fs_redirs(const char* game_data_path_abs)
{
    ...
    // Sometimes, the game decides to read stuff using lower case, sometimes upper case *shrug*
    // Also, we don't are about cleaning up the resulting merged strings here. Have some memory leaks!
    ...
    cnh_redir_add("stage.cfg", util_str_merge(game_data_path_abs, "/STAGE.CFG"));
    cnh_redir_add("STAGE.CFG", util_str_merge(game_data_path_abs, "/STAGE.CFG"));
    // Special treatment for extra because keeping the name stage.cfg was not good enough...
...
static void mk3hook_patch_1st_and_2nd_fs_init(struct mk3hook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
static void mk3hook_patch_main_loop_init(struct mk3hook_options* options)
{
    ...
    patch_main_loop_init(true, options->patch.main_loop.disable_built_in_inputs, true);
...
static void mk3hook_patch_sound_init(struct mk3hook_options* options)
{
    ...
    mk3hook_fmodex_init(options->patch.sound.debug_output, options->patch.sound.device);
}
