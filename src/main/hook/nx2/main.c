...
static void nx2hook_bootstrapping(int argc, char** argv, struct nx2hook_options* options, char** game_data_path)
{
    log_assert(argc >= 0);
    log_assert(argv);
    log_assert(options);
...
static void nx2hook_fs_redir_script(struct nx2hook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
static void nx2hook_fs_redir_fonts(struct nx2hook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
static void nx2hook_fs_redir_mission(struct nx2hook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
static void nx2hook_fs_redirs_init(struct nx2hook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
{
    patch_microdog40_init((const uint8_t*) _binary_nx2hook_dog_key_start,
        ((uintptr_t) &_binary_nx2hook_dog_key_end - (uintptr_t) &_binary_nx2hook_dog_key_start));
}

static void nx2hook_patch_hdd_check_init()
{
    patch_hdd_check_init((const uint8_t*) _binary_nx2hook_hdd_raw_start,
        ((uintptr_t) &_binary_nx2hook_hdd_raw_end - (uintptr_t) &_binary_nx2hook_hdd_raw_start));
}
