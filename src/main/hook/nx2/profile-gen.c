...
nx2hook_profile_gen_filehook(struct cnh_filehook_irp *irp)
{
  ...
  if (irp->op != CNH_FILEHOOK_IRP_OP_OPEN ||
      nx2hook_profile_gen_is_creating_file) {
  ...
  if (util_str_ends_with(irp->open_filename, "nx2save.bin") &&
    ...
    if (!asset_nx2_usb_save_save_to_file(irp->open_filename, save, true)) {
      log_error(
          "Saving generated nx2save.bin file to %s failed", irp->open_filename);
    }
  ...
  if (util_str_ends_with(irp->open_filename, "nx2rank.bin") &&
    ...
    if (!asset_nx2_usb_rank_save_to_file(irp->open_filename, rank, true)) {
      log_error(
          "Saving generated nx2rank.bin file to %s failed", irp->open_filename);
    }
