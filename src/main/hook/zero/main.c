...
static void zerohook_bootstrapping(int argc, char** argv, struct zerohook_options* options, char** game_data_path)
{
    log_assert(argc >= 0);
    log_assert(argv);
    log_assert(options);
...
static void zerohook_fs_redir_script(struct zerohook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
static void zerohook_fs_redirs_init(struct zerohook_options* options, const char* game_data_path)
{
    log_assert(options);
    log_assert(game_data_path);
...
{
    patch_microdog34_init((const uint8_t*) _binary_zerohook_dog_key_start,
        ((uintptr_t) &_binary_zerohook_dog_key_end - (uintptr_t) &_binary_zerohook_dog_key_start));
}

static void zerohook_patch_hdd_check_init()
{
    patch_hdd_check_init((const uint8_t*) _binary_zerohook_hdd_raw_start,
        ((uintptr_t) &_binary_zerohook_hdd_raw_end - (uintptr_t) &_binary_zerohook_hdd_raw_start));
}
