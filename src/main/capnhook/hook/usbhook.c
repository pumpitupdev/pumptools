...
typedef int (*cnh_usbhook_usb_set_altinterface_t)(
    usb_dev_handle *dev, int interface);
typedef int (*cnh_usbhook_usb_set_configuration_t)(
    usb_dev_handle *dev, int configuration);
typedef int (*cnh_usbhook_usb_claim_interface_t)(
    usb_dev_handle *dev, int interface);
typedef int (*cnh_usbhook_usb_control_msg_t)(
    usb_dev_handle *dev,
    int requesttype,
    int request,
    int value,
...
static enum cnh_result _cnh_usbhook_invoke_next_reset_advance(
    struct cnh_usbhook_irp *irp, bool reset_next_handler_advance);

static enum cnh_result _cnh_usbhook_invoke_real(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_init(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_find_busses(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_find_devices(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_open(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_close(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_reset(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_set_altinterface(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_set_configuration(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_claim_interface(struct cnh_usbhook_irp *irp);
static enum cnh_result
_cnh_usbhook_invoke_real_ctrl_msg(struct cnh_usbhook_irp *irp);
...
static const cnh_usbhook_fn_t _cnh_usbhook_real_handlers[10] = {
    ...
    [CNH_USBHOOK_IRP_OP_SET_ALTINTERFACE] =
        _cnh_usbhook_invoke_real_set_altinterface,
    [CNH_USBHOOK_IRP_OP_SET_CONFIGURATION] =
        _cnh_usbhook_invoke_real_set_configuration,
    [CNH_USBHOOK_IRP_OP_CLAIM_INTERFACE] =
        _cnh_usbhook_invoke_real_claim_interface,
...
enum cnh_result cnh_usbhook_push_handler(cnh_usbhook_fn_t fn)
{
  ...
  new_array =
      realloc(_cnh_usbhook_handlers, new_size * sizeof(cnh_usbhook_fn_t));
...
static void _cnh_usbhook_init(void)
{
  ...
  if (!atomic_compare_exchange_strong(
          &_cnh_usbhook_init_in_progress, &expected, 1)) {
  ...
  _cnh_usbhook_real_init =
      (cnh_usbhook_usb_init_t) cnh_lib_get_func_addr("usb_init");
  _cnh_usbhook_real_find_devices =
      (cnh_usbhook_usb_find_devices_t) cnh_lib_get_func_addr(
          "usb_find_devices");
  _cnh_usbhook_real_find_busses =
      (cnh_usbhook_usb_find_busses_t) cnh_lib_get_func_addr("usb_find_busses");
  _cnh_usbhook_real_open =
      (cnh_usbhook_usb_open_t) cnh_lib_get_func_addr("usb_open");
  _cnh_usbhook_real_close =
      (cnh_usbhook_usb_close_t) cnh_lib_get_func_addr("usb_close");
  _cnh_usbhook_real_reset =
      (cnh_usbhook_usb_reset_t) cnh_lib_get_func_addr("usb_reset");
  _cnh_usbhook_real_set_altinterface =
      (cnh_usbhook_usb_set_altinterface_t) cnh_lib_get_func_addr(
          "usb_set_altinterface");
  _cnh_usbhook_real_set_configuration =
      (cnh_usbhook_usb_set_configuration_t) cnh_lib_get_func_addr(
          "usb_set_configuration");
  _cnh_usbhook_real_claim_interface =
      (cnh_usbhook_usb_claim_interface_t) cnh_lib_get_func_addr(
          "usb_claim_interface");
  _cnh_usbhook_real_control_msg =
      (cnh_usbhook_usb_control_msg_t) cnh_lib_get_func_addr("usb_control_msg");
...
_cnh_usbhook_invoke_real_set_configuration(struct cnh_usbhook_irp *irp)
{
  ...
  res =
      _cnh_usbhook_real_set_configuration(irp->handle, irp->set_configuration);
...
_cnh_usbhook_invoke_real_ctrl_msg(struct cnh_usbhook_irp *irp)
{
  ...
  res = _cnh_usbhook_real_control_msg(
      irp->handle,
      irp->ctrl_req_type,
      irp->ctrl_req,
      irp->ctrl_value,
      irp->ctrl_index,
      (char *) irp->ctrl_buffer.bytes,
