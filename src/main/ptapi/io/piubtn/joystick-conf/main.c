...
static void menu_joystick_test()
{
    ...
    for (size_t i = 0; i < connected_joysticks; i++) {
        printf("%d: %s -> %s, buttons %d, axes %d\n",
            i, dev_info[i].name, dev_info[i].dev_path, dev_info[i].num_buttons, dev_info[i].num_axes);
    }
    ...
    if (!io_util_joystick_open(dev_info[selected_device_idx].dev_path, &joystick)) {
        printf("Error opening device %s -> %s", dev_info[selected_device_idx].name,
...
{
    struct ptapi_io_piubtn_joystick_util_conf* config;
    struct ptapi_io_piubtn_joystick_util_conf_entry config_entry;
    ...
    if (!ptapi_io_piubtn_joystick_util_conf_read_from_file(&config, CONFIG_FILENAME)) {
        printf("Reading existing config file failed, creating new\n");

        ptapi_io_piubtn_joystick_util_conf_new(&config);

        memset(&config_entry, 0, sizeof(struct ptapi_io_piubtn_joystick_util_conf_entry));
        ...
        if (!ptapi_io_piubtn_joystick_util_conf_find_entry(config, dev_info[selected_device_idx].name,
                dev_info[selected_device_idx].dev_path, &config_entry)) {
            printf("Could not find existing mapping entry in config for joystick, creating new one\n");
    ...
    printf("Configuring device %s -> %s\n", dev_info[selected_device_idx].name, dev_info[selected_device_idx].dev_path);
    ...
    for (uint32_t i = PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P1_LEFT;
            i < PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_COUNT;
            i++) {
        printf("Press button to assign to \"%s\" or Enter/Return to skip: ", ptapi_io_piubtn_joystick_util_conf_input_str[i]);
        fflush(stdout);
    ...
    printf("Configuration of device %s -> %s done\n", dev_info[selected_device_idx].name,
        dev_info[selected_device_idx].dev_path);

    if (!ptapi_io_piubtn_joystick_util_conf_set_entry(config, &config_entry)) {
        ptapi_io_piubtn_joystick_util_conf_append_entry(&config, &config_entry);
    }

    ptapi_io_piubtn_joystick_util_conf_write_to_file(config, CONFIG_FILENAME);
