...
const char* ptapi_io_piubtn_keyboard_util_conf_input_str[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_COUNT] = {
    "invalid",
    "P1 Left",
    "P1 Right",
    "P1 Start",
...
bool ptapi_io_piubtn_keyboard_util_conf_write_to_file(struct ptapi_io_piubtn_keyboard_util_conf* conf, const char* path)
{
    ...
    return util_file_save(path, conf, sizeof(struct ptapi_io_piubtn_keyboard_util_conf));
}