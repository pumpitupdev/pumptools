...
{
  struct ptapi_io_piubtn_keyboard_util_conf *config;

  if (!ptapi_io_piubtn_keyboard_util_conf_read_from_file(
          &config, CONFIG_FILENAME)) {
    printf("Reading existing config file failed, creating new\n");

    ptapi_io_piubtn_keyboard_util_conf_new(&config);
  } else {
    printf(
        "Found existing mapping entry in config for keyboard, overwriting\n");
  }

  printf(
      "A new window with no contents/white background is opened. Ensure to "
      "have the application focus on that "
      "window to allow input capturing. Otherwise, the tool cannot capture any "
      "keyboard inputs!\n");

  // Delay a moment to avoid picking up any previous keyboard inputs
  util_time_sleep_ms(500);

  for (uint32_t i = PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P1_LEFT;
       i < PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_COUNT;
       i++) {
    printf(
        "Press button to assign to \"%s\": ",
        ptapi_io_piubtn_keyboard_util_conf_input_str[i]);
    fflush(stdout);
  ...
  ptapi_io_piubtn_keyboard_util_conf_write_to_file(config, CONFIG_FILENAME);
...
int main(int argc, char **argv)
{
  ...
  printf("ptapi PIUBTN keyboard button mapping config tool\n");
