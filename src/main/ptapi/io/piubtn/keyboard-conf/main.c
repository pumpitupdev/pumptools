...
static void menu_keyboard_map()
{
    ...
    if (!ptapi_io_piubtn_keyboard_util_conf_read_from_file(&config, CONFIG_FILENAME)) {
    ...
    for (uint32_t i = PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P1_LEFT;
            ...
            i++) {
        printf("Press button to assign to \"%s\": ", ptapi_io_piubtn_keyboard_util_conf_input_str[i]);
        fflush(stdout);
