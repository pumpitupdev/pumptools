...
{
  memset(piubtn_drv_piubtn_out_buffer, 0, sizeof(piubtn_drv_piubtn_out_buffer));
...
bool ptapi_io_piubtn_recv(void)
{
  if (!piubtn_drv_device_read(
          piubtn_drv_piubtn_in_buffer, sizeof(piubtn_drv_piubtn_in_buffer))) {
    return false;
  }
...
bool ptapi_io_piubtn_send(void)
{
  ...
  if (!piubtn_drv_device_write(
          piubtn_drv_piubtn_out_buffer, sizeof(piubtn_drv_piubtn_out_buffer))) {
    return false;
  }
