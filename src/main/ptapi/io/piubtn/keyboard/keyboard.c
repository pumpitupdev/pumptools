...
static const struct ptapi_io_x11_input_hook_handler
    _piubtn_emu_keyboard_input_hook_handler = {
    ...
    uint8_t player, struct ptapi_io_piubtn_inputs *inputs)
{
  if (player == 0) {
    inputs->left = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P1_LEFT]];
    inputs->right = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P1_RIGHT]];
    inputs->start = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P1_START]];
    inputs->back = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P1_BACK]];
  } else {
    inputs->left = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P2_LEFT]];
    inputs->right = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P2_RIGHT]];
    inputs->start = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P2_START]];
    inputs->back = ptapi_io_piubtn_keyboard_buffer
        [ptapi_io_piubtn_keyboard_conf
             ->button_map[PTAPI_IO_PIUBTN_KEYBOARD_UTIL_CONF_INPUT_P2_BACK]];
  }
}
