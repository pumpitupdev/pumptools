...
static struct ptapi_io_piubtn_joystick_util_conf* ptapi_io_piubtn_joystick_util_conf;
static struct ptapi_io_piubtn_joystick_with_conf ptapi_io_piubtn_joysticks_with_conf[MAX_NUM_JOYSTICKS];
...
bool ptapi_io_piubtn_open()
{
   char path[PATH_MAX];
    ...
    if (!ptapi_io_piubtn_joystick_util_conf_read_from_file(&ptapi_io_piubtn_joystick_util_conf, config_path)) {
    ...
    memset(ptapi_io_piubtn_joysticks_with_conf, 0, sizeof(struct ptapi_io_piubtn_joystick_with_conf) * MAX_NUM_JOYSTICKS);
    ...
    for (size_t i = 0; i < devices_connected; i++) {
        ...
        for (size_t j = 0; j < ptapi_io_piubtn_joystick_util_conf->num_entries; j++) {

            if (!strcmp(ptapi_io_piubtn_joystick_util_conf->entries[j].name, device_info[i].name) &&
                    !strcmp(ptapi_io_piubtn_joystick_util_conf->entries[j].dev_path, device_info[i].dev_path)) {
                log_debug("Found matching configuration for %s -> %s", device_info[i].name, device_info[i].dev_path);

                ptapi_io_piubtn_joysticks_with_conf[ptapi_io_piubtn_num_active_joysticks].conf =
                        &ptapi_io_piubtn_joystick_util_conf->entries[j];
    ...
    for (size_t i = 0; i < ptapi_io_piubtn_num_active_joysticks; i++) {
        ...
        if (!io_util_joystick_open(ptapi_io_piubtn_joysticks_with_conf[i].conf->dev_path,
                &ptapi_io_piubtn_joysticks_with_conf[i].joystick)) {
            log_warn("Could not open joystick %s -> %s, ignoring", ptapi_io_piubtn_joysticks_with_conf[i].conf->name,
                    ptapi_io_piubtn_joysticks_with_conf[i].conf->dev_path);
        } else {
            log_info("Opened joystick %s -> %s", ptapi_io_piubtn_joysticks_with_conf[i].conf->name,
                    ptapi_io_piubtn_joysticks_with_conf[i].conf->dev_path);
        }
    }
...
bool ptapi_io_piubtn_recv(void)
{
    ...
    for (size_t i = 0; i < ptapi_io_piubtn_num_active_joysticks; i++) {
        if (ptapi_io_piubtn_joysticks_with_conf[i].joystick) {
            success = io_util_joystick_update(ptapi_io_piubtn_joysticks_with_conf[i].joystick);

            if (!success) {
                log_error("Updating joystick %s -> %s failed", ptapi_io_piubtn_joysticks_with_conf[i].conf->name,
                        ptapi_io_piubtn_joysticks_with_conf[i].conf->dev_path);
            }
        }
    }
...
{
    if (player == 0) {
        inputs->start = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P1_START);
        inputs->back = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P1_BACK);
        inputs->left = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P1_LEFT);
        inputs->right = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P1_RIGHT);
    } else {
        inputs->start = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P2_START);
        inputs->back = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P2_BACK);
        inputs->left = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P2_LEFT);
        inputs->right = ptapi_io_piubtn_merge_inputs(PTAPI_IO_PIUBTN_JOYSTICK_UTIL_CONF_INPUT_P2_RIGHT);
    }
}
