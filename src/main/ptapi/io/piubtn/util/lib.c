    ...
    const char *path, struct ptapi_io_piubtn_api *api)
{
  ...
  api->get_input = (ptapi_io_piubtn_get_input_t) cnh_lib_get_func_addr_handle(
      handle, "ptapi_io_piubtn_get_input");
  api->set_output = (ptapi_io_piubtn_set_output_t) cnh_lib_get_func_addr_handle(
      handle, "ptapi_io_piubtn_set_output");
