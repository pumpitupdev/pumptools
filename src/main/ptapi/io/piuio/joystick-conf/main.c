...
static void menu_joystick_map()
{
    ...
    if (!ptapi_io_piuio_joystick_util_conf_read_from_file(&config, CONFIG_FILENAME)) {
        ...
        if (!ptapi_io_piuio_joystick_util_conf_find_entry(config, dev_info[selected_device_idx].name,
    ...
    for (uint32_t i = PTAPI_IO_PIUIO_JOYSTICK_UTIL_CONF_INPUT_P1_LU; i < PTAPI_IO_PIUIO_JOYSTICK_UTIL_CONF_INPUT_COUNT;
            i++) {
        printf("Press button to assign to \"%s\" or Enter/Return to skip: ", ptapi_io_piuio_joystick_util_conf_input_str[i]);
        fflush(stdout);
