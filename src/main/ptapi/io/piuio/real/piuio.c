...
void ptapi_io_piuio_get_input_pad(uint8_t player,
        enum ptapi_io_piuio_sensor_group sensor_group,
        struct ptapi_io_piuio_pad_inputs* inputs)
{
    memcpy(inputs, &piuio_drv_piuio_pad_in[player][sensor_group],
        sizeof(struct ptapi_io_piuio_pad_inputs));
}

void ptapi_io_piuio_get_input_sys(struct ptapi_io_piuio_sys_inputs* inputs)
{
    memcpy(inputs, &piuio_drv_piuio_sys,
        sizeof(struct ptapi_io_piuio_sys_inputs));
}

void ptapi_io_piuio_set_output_pad(uint8_t player,
        const struct ptapi_io_piuio_pad_outputs* outputs)
{
    memcpy(&piuio_drv_piuio_pad_out[player], outputs,
...
{
    memcpy(&piuio_drv_piuio_cab_out, outputs,
